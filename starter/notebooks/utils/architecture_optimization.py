...
def create_optimized_model(
    ...
    optimization_order = [
        "interpolation_removal",  # First: Changes input processing
        "channel_optimization",  # Second: Hardware-level optimization
        "depthwise_separable",  # Third: Layer structure changes (if you implement it)
        "grouped_conv",  # Alternative to depthwise
        "inverted_residuals",  # Fourth: Block-level changes
        "lowrank_factorization",  # Fifth: Parameter compression
        "parameter_sharing",  # Last: Cross-layer optimization
    ]
...
def apply_interpolation_removal_optimization(
    ...
    # Add your code here
    # Update target_size to 64 so the model doesn't upscale
...
def apply_channel_optimization(
    ...
    # Add your code here
    # 1. Convert to channels_last memory format for better GPU efficiency
    if enable_channels_last:
        optimized_model = optimized_model.to(memory_format=torch.channels_last)

    # 2. Enable in-place ReLU to save memory
