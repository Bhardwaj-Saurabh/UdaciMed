...
class ClassificationEvaluator:
    ...
    def __init__(self, device: str = "cuda", threshold: float = 0.5) -> None:
        ...
        self.confusion_matrix = BinaryConfusionMatrix(threshold=threshold).to(
            self.device
        )
...
def find_optimal_threshold(
    ...
    print(
        f"Optimal threshold: {best_threshold:.3f} (optimizing {metric}: {best_score:.3f})"
    )
...
def format_evaluation_results(metrics: Dict[str, Union[float, np.ndarray]]) -> str:
    ...
    threshold_info = (
        f" (threshold: {metrics.get('threshold', 0.5):.3f})"
